(function (exports, Laya) {
  ...
  class AnimationParser01 {
    static parse(templet, reader, callback) {
      ...
      let check = ()=>{
        if (pro == i) {//处理好了，不然就等下一次
          pro++;
          deal();
        } else {
          console.warn("解析动画卡顿：", i);
        }
      }
      let deal = (index)=>{
        ...
        for (j = 0; j < boneCount; j++) {
          ...
          for (k = 0, n = keyframeCount; k < n; k++) {
            ...
            /** lc **/
            keyFrame.data = reader.readFloat32Array(reader._pos_, keyframeDataCount * 4);
            /*
        ...
        if (i >= aniCount) {
          ...
          // if (index < 2) {
          //   Laya.timer.frameOnce(1, this, deal, [0])
          // } else {
          //   deal(index);
          // }
        }
  ...
  class AnimationTemplet extends Laya.Resource {
    ...
    /** lc **/
    static _LinearInterpolation_0(bone, index, out, outOfs, data, dt, duration, nextData, interData = null) {
      var rate = duration === 0 ? 0 : dt / duration;
      out[outOfs] = data[index] * (1 - rate) + nextData[index] * rate;
      return 1;
    }
    static _QuaternionInterpolation_1(bone, index, out, outOfs, data, dt, duration, nextData, interData = null) {
      var amount = duration === 0 ? 0 : dt / duration;
      Laya.MathUtil.slerpQuaternionArray(data, index, nextData, index, amount, out, outOfs);
      return 4;
    }
    static _AngleInterpolation_2(bone, index, out, outOfs, data, dt, duration, nextData, interData = null) {
      return 0;
    }
    static _RadiansInterpolation_3(bone, index, out, outOfs, data, dt, duration, nextData, interData = null) {
      return 0;
    }
    static _Matrix4x4Interpolation_4(bone, index, out, outOfs, data, dt, duration, nextData, interData = null) {
      var r = duration === 0 ? 0 : dt / duration;
      var f = 1 - r;
      for (var i = 0; i < 16; i++ , index++)
        out[outOfs + i] = data[index] * f + nextData[index] * r;
      return 16;
    }
    static _NoInterpolation_5(bone, index, out, outOfs, data, dt, duration, nextData, interData = null) {
      out[outOfs] = data[index];
      return 1;
    }
    static _BezierInterpolation_6(bone, index, out, outOfs, data, dt, duration, nextData, interData = null, offset = 0) {
      out[outOfs] = data[index] + (nextData[index] - data[index]) * BezierLerp.getBezierRate(dt / duration, interData[offset], interData[offset + 1], interData[offset + 2], interData[offset + 3]);
      return 5;
    }
    static _BezierInterpolation_7(bone, index, out, outOfs, data, dt, duration, nextData, interData = null, offset = 0) {
    ...
    _calculateKeyFrame(node, keyframeCount, keyframeDataCount) {
      ...
      var len = keyframeCount - 1;
      for (var i = 0; i < len; i++) {
        keyFrames[i].nextData = keyFrames[i + 1].data;
      }
      keyFrames[len].nextData = keyFrames[0].data;
    }
          ...
          switch (lerpType) {
            ...
            case 1:
              for (j = 0; j < node.keyframeWidth;)
                j += node.interpolationMethod[j](node, j, originalData, outOfs + j, key.data, dt, key.duration, key.nextData);
              break;
            case 2:
              ...
              for (j = 0; j < interDataLen;) {
                ...
                switch (type) {
                  case 6:
                    j += AnimationTemplet.interpolation[type](node, dataIndex, originalData, outOfs + dataIndex, key.data, dt, key.duration, key.nextData, interpolationData, j + 1);
                    break;
                  case 7:
                    j += AnimationTemplet.interpolation[type](node, dataIndex, originalData, outOfs + dataIndex, key.data, dt, key.duration, key.nextData, interpolationData, j + 1);
                    break;
                  default:
                    j += AnimationTemplet.interpolation[type](node, dataIndex, originalData, outOfs + dataIndex, key.data, dt, key.duration, key.nextData);
                }
                dataIndex++;
              }
              break;
  ...
  class GraphicsAni extends Laya.Graphics {
    //todo blendMode
    drawSkin(skinA, alpha, blendMode) {
      this.drawTriangles(skinA.texture, 0, 0, skinA.vertices, skinA.uvs, skinA.indexes, skinA.transform || Laya.Matrix.EMPTY, alpha, null, blendMode);
    }
    static create() {
  ...
  class BoneSlot {
    ...
    draw(graphics, boneMatrixArray, noUseSave = false, alpha = 1) {
      ...
      switch (this.currDisplayData.type) {
          ...
          if (graphics) {
            ...
            if (this._parentMatrix) {
              ...
              if (tCurrentMatrix) {
                ...
                if (tRotateKey) {
                  //todo blendMode
                  graphics.drawTexture(tTexture, -this.currDisplayData.height / 2, -this.currDisplayData.width / 2, this.currDisplayData.height, this.currDisplayData.width, tResultMatrix, alpha, null, this.blendMode);
                }
                else {
                  graphics.drawTexture(tTexture, -this.currDisplayData.width / 2, -this.currDisplayData.height / 2, this.currDisplayData.width, this.currDisplayData.height, tResultMatrix, alpha, null, this.blendMode);
                }
              }
            }
          }
        ...
        case 1:
          ...
          //todo blendMode
          graphics.drawSkin(tSkinSprite, alpha, this.blendMode);
          break;
        case 2:
          if (noUseSave) {
            if (this._skinSprite == null) {
    ...
    copy() {
      ...
      tBoneSlot.blendMode = this.blendMode;
      return tBoneSlot;
    }
  }
  ...
  class Skeleton extends Laya.Sprite {
    ...
    _createGraphics(_clipIndex = -1) {
      ...
      if (this._drawOrder) {
        for (i = 0, n = this._drawOrder.length; i < n; i++) {
          ...
          this.recordSlot(tDBBoneSlot,tSlotData3,this._aniClipIndex, _clipIndex);
        }
      }
      else {
        for (i = 0, n = this._boneSlotArray.length; i < n; i++) {
    ...
    recordSlot(slot,alpha,aniIndex, frameIndex){
      if(slot.currSlotData != null)return;
      if(slot.parent != slot.name)return;//特殊规则，只有插槽和骨骼同名，并且图片为空才是设计的挂点
      if(this._templet.linkData == null){
        this._templet.linkData = {};
      }
      var linkData = this._templet.linkData;
      if(linkData[slot.name] == null){
        linkData[slot.name] = {};
      }
      var m = slot._parentMatrix;
      let scaleX = m.getScaleX();
      let v = Math.acos(m.a/scaleX);
      let rotation = (m.b>0?v:-v)*180/Math.PI;
      linkData[slot.name][frameIndex*100+aniIndex] = {x:m.tx,y:m.ty,scaleX:scaleX,scaleY:m.getScaleY(),alpha:alpha,rotation:rotation};
    }
    ...
    getBoneCoords(animName, boneName) {
      var arrCoords = [];
      var aniClipIndex = this._templet._aniMap[animName];
      var curOriginalData = new Float32Array(this._templet.getTotalkeyframesLength(aniClipIndex));

      var interval = 1000.0 / this._player.cacheFrameRate;
      var playTime = this._templet.getAniDuration(aniClipIndex);
      var lenJ = playTime / interval;

      for (var j = 0; j < lenJ; j++)
      {
        // var curTime = j * this._player.cacheFrameRateInterval;
        // this._templet.getOriginalData(aniClipIndex, curOriginalData, this._templet._fullFrames[aniClipIndex], j, curTime);

        var tStartIndex = 0;
        var tParentTransform;
        var tSrcBone;
        var boneCount = this._templet.srcBoneMatrixArr.length;
        var tSectionArr = this._aniSectionDic[aniClipIndex];
        var i = 0, n = 0;
        for (i = 0, n = tSectionArr[0]; i < boneCount; i++)
        {
          tSrcBone = this._boneList[i];
          if (tSrcBone.name == boneName) {
            tParentTransform = this._templet.srcBoneMatrixArr[i];
            // var boneX =/* tParentTransform.x + */curOriginalData[tStartIndex + 4] * 0.35;
            // var boneY =/* tParentTransform.y + */curOriginalData[tStartIndex + 5] * 0.35;
            arrCoords[j] = curOriginalData.slice(tStartIndex, tStartIndex + this._templet.tMatrixDataLen);
            if (tSrcBone.parentBone) {
              // console.warn("击飞骨骼矩阵", tSrcBone.resultMatrix);

              arrCoords[j][0] *=  tSrcBone.parentBone.resultMatrix.a;//scX
              arrCoords[j][1] +=  tSrcBone.parentBone.resultMatrix.b;//skX
              arrCoords[j][2] +=  tSrcBone.parentBone.resultMatrix.c;//skY
              arrCoords[j][3] *=  tSrcBone.parentBone.resultMatrix.d;//scX
              arrCoords[j][4] +=  tSrcBone.parentBone.resultMatrix.tx;//x
              arrCoords[j][5] +=  tSrcBone.parentBone.resultMatrix.ty;//y
              if (this._templet.tMatrixDataLen === 8) {
                arrCoords[j][6] += tSrcBone.parentBone.resultTransform.skewX;//skewX
                arrCoords[j][7] += tSrcBone.parentBone.resultTransform.skewY;//skewY
              }
              // Laya.Matrix.mul(tCurrentMatrix, tSrcBone.resultMatrix., Laya.Matrix.TEMP)
            }
            // console.warn("动作帧数", arrCoords[j]);
            break;
          }
          tStartIndex += this._templet.tMatrixDataLen;
        }
      }
      return arrCoords;
    }
    ...
    play(nameOrIndex, loop, force = true, start = 0, end = 0, freshSkin = true, playAudio = true) {
      ...
      if (index > -1 && index < this.getAnimNum()) {
        ...
        var errorMsg = [];
        for (var ti = 0, tn = this._templet.getAnimationCount(); ti < tn; ti++) {
          var animation = this._templet.getAnimation(ti);
          if (animation) {
            errorMsg.push(animation.name)
          }
        }
        console.log(this._templet._skBufferUrl+' "'+nameOrIndex+'" no exist:'+errorMsg.join(','));
      }
    }
    stop() {
      if (!this._pause) {
  ...
  class Templet extends AnimationTemplet {
    ...
    parse(data) {
      //super.parse(data);
      var reader = new Laya.Byte(data);
      var _aniVersion = this._aniVersion = reader.readUTFString();
      if (_aniVersion == "LAYAANIMATION:03") {
        this.parseSk(reader);
        if (this._mainTexture) {
          this._textureComplete03();
        } else {
          Laya.loader.load(this._loadList, Laya.Handler.create(this, this._textureComplete03));
        }
      } else {
        // console.warn("解析动画数据", new Date().getTime())
        AnimationParser01.parse(this, reader, ()=>{
          this.event(Laya.Event.LOADED, this);
          if (_aniVersion === Templet.LAYA_ANIMATION_VISION) {
            this._isParseAudio = true;
          } else if (_aniVersion != Templet.LAYA_ANIMATION_160_VISION) {
            console.log("[Error] 版本不一致，请使用IDE版本配套的重新导出" + _aniVersion + "->" + Templet.LAYA_ANIMATION_VISION);
          }
          if (this._mainTexture) {
            this._parsePublicExtData();
          } else {
            // let isLoad = true;
            // for (let i = 0, len = this._loadList.length; i<len; i++) {
            //   let tTextureName = this._loadList[i];
            //   if (!this._textureDic[tTextureName]) {
            //     isLoad = false;
            //     break;
            //   }
            // }
            // if (isLoad) {
            //   this._parsePublicExtData();
            //   console.warn("解析图集", new Date().getTime())
            // }

            this._parseTexturePath();
          }
        });
        // if (!this._mainTexture) {//同时加载资源
        //   this._parseTexturePath();
        // }
    ...
    parseSk(reader) {
      this.reader = reader;
      this._aniClassName = reader.readUTFString();

      this._loadList = [this._path + reader.readUTFString()];

      var str = this.readStr(reader);
      this.strList = str.split("\n");
      var dataView = reader._d_;
      var pos = reader._pos_;

      var arrayTotal = dataView.getUint32(pos, true);
      pos += 4;
      var arrayChunk = dataView.buffer.slice(pos, pos + arrayTotal);
      pos += arrayTotal;
      var arrayListLen = dataView.getUint16(pos, true);
      pos += 2
      var arrayList = new Array(arrayListLen);

      var offset = 0;
      for (var i = 0; i < arrayListLen; i++) {
        var type_num = dataView.getUint16(pos, true);
        pos += 2;
        var type = type_num >>> 14;
        var num = type_num & 0x3fff;
        if (type == 3) {
          arrayList[i] = new Float32Array(arrayChunk, offset, num);
          offset += num * 4;
        } else if (type == 2) {
          arrayList[i] = new Int16Array(arrayChunk, offset, num);
          if (num % 2 == 0) {
            offset += num * 2;
          } else {
            offset += (num + 1) * 2;
          ...
          if (type == 1) {
            arrayList[i] = new Int8Array(arrayChunk, offset, num);
          } else {
            arrayList[i] = new Uint8Array(arrayChunk, offset, num);
          }
          offset += num;
          let remain = num % 4;
          if (remain > 0) {
    ...
    _parseAni() {
      ...
      var u8 = reader._u8d_;
      var pos = reader._pos_;
      var strList = this.strList;
      var arrayList = this.arrayList;

      if (Templet.slotInterpolationMethod == null) {
        Templet.slotInterpolationMethod = [AnimationTemplet.interpolation[5],
          AnimationTemplet.interpolation[0], AnimationTemplet.interpolation[0],
          AnimationTemplet.interpolation[0], AnimationTemplet.interpolation[0],
          AnimationTemplet.interpolation[0]];
      }
      var aniCount = dataView.getUint8(pos++, true);
      var matrixDataLen = this.tMatrixDataLen = u8[pos++];
      var boneNum = u8[pos++];
      for (var i = 0; i < aniCount; i++) {
        var ani = this._anis[i] = new AnimationContent();
        var nameIndex = this.readVaruint(pos);
        var name = ani.name = strList[nameIndex];
        pos = reader._pos_;
        this._aniMap[name] = i;
        ani.bone3DMap = {};
        ani.playTime = dataView.getFloat32(pos, true);
        pos += 4;
        var boneCount = u8[pos++];
        ani.nodes = new Array(boneCount);
        ani.totalKeyframeDatasLength = 0;
        for (var j = 0; j < boneCount; j++) {
          ...
          node.name = strList[nameIndex];
          ani.bone3DMap[node.name] = j;
          var parentIndex = u8[pos++];
          node.parent = parentIndex == 255 ? null : ani.nodes[parentIndex];
          node.lerpType = j >= boneNum ? 1 : 2;
          var keyframeDataCount = node.keyframeWidth = node.lerpType == 1 ? 6 : matrixDataLen;
          ani.totalKeyframeDatasLength += keyframeDataCount;
          if (node.lerpType === 0 || node.lerpType === 1) {
            node.interpolationMethod = Templet.slotInterpolationMethod;
          }
          if (node.parent != null)
            node.parent.childs.push(node);
          var keyframeCount = u8[pos++];
          node.keyFrame = new Array(keyframeCount);
          for (var k = 0, n = keyframeCount - 1; k <= n; k++) {
            ...
            if (k == 0) {
              ...
              if (k == n) {
                ...
                keyFrame.startTime = dataView.getFloat32(pos, true);
                pos += 4;
              }
              node.keyFrame[k - 1].duration = keyFrame.startTime - node.keyFrame[k - 1].startTime;
            }
            if (node.lerpType === 2) {
              keyFrame.interpolationData = arrayList[this.readVaruint(pos)];
              pos = reader._pos_;
            }
            var arrayIndex = this.readVaruint(pos);
            keyFrame.data = arrayList[arrayIndex];
            pos = reader._pos_;
          }
          node.playTime = ani.playTime;
    ...
    _parsePublicExtData03() {
      var i = 0, j = 0, k = 0, l = 0;
      var aniCount = this._anis.length;
      for (i = 0; i < aniCount; i++) {
        this._graphicsCache.push([]);
      };
      var isSpine = false;
      isSpine = this._aniClassName != "Dragon";
      var tByte = this.reader;
      var strList = this.strList;
      var arrayList = this.arrayList;
      var tBoneLen = tByte.readUint16();
      for (i = 0; i < aniCount; i++) {
        this.aniSectionDic[i] = arrayList[this.readVaruint()];
      };
      ...
      for (i = 0; i < tBoneLen; i++) {
        ...
        tBone.d = isSpine ? -1 : 1;
        var tName = strList[this.readVaruint()];
        var tParentNameIndex = tByte.readUint8();
        tBone.length = tByte.readFloat32();
        var inherit = tByte.readUint8();
        if ((inherit & 0x1) != 0) {
        ...
        tBone.name = tName;
        if (tParentNameIndex != 0xff) {
          this.mBoneArr[tParentNameIndex].addChild(tBone);
        } else {
          this.mRootBone = tBone;
        }
        var tResultTransform = this.readTransform(tByte);
        tMatrixArray[i] = tResultTransform;
        tBone.transform = tResultTransform;
        this.mBoneArr[i] = tBone;
      };
      var tIkConstraintData;
      var tIkLen = tByte.readUint16();
      for (i = 0; i < tIkLen; i++) {
        tIkConstraintData = new IkConstraintData();
        tIkConstraintData.boneIndexs = arrayList[this.readVaruint()];
        tIkConstraintData.name = strList[this.readVaruint()];
        tIkConstraintData.targetBoneName = strList[this.readVaruint()];
        tIkConstraintData.targetBoneIndex = tByte.readInt16();
        tIkConstraintData.bendDirection = tByte.readInt16();
      ...
      for (i = 0; i < tTfLen; i++) {
        ...
        tTfConstraintData.name = strList[this.readVaruint()];
      ...
      for (i = 0; i < tPathLen; i++) {
        ...
        tPathConstraintData.name = strList[this.readVaruint()];
        tPathBoneLen = tByte.readUint16();
        for (j = 0; j < tPathBoneLen; j++) {
          tPathConstraintData.bones.push(tByte.readInt16());
        }
        tPathConstraintData.target = strList[this.readVaruint()];
        tPathConstraintData.positionMode = strList[this.readVaruint()];
        tPathConstraintData.spacingMode = strList[this.readVaruint()];
        tPathConstraintData.rotateMode = strList[this.readVaruint()];
      ...
      for (i = 0; i < tDeformAniLen; i++) {
        ...
        for (var f = 0; f < tDeformSkinLen; f++) {
          ...
          tDeformAniData.skinName = strList[this.readVaruint()];
          ...
          for (j = 0; j < tDeformSlotLen; j++) {
            ...
            for (k = 0; k < tDeformSlotDisplayLen; k++) {
              ...
              tDeformSlotDisplayData.attachment = strList[this.readVaruint()];
              tDeformTimeLen = tByte.readInt16();
              tDeformSlotDisplayData.timeList = arrayList[this.readVaruint()];
              for (l = 0; l < tDeformTimeLen; l++) {
                if (tByte.readUint8() == 1) {
                  tDeformSlotDisplayData.tweenKeyList.push(true);
                } else {
                  tDeformSlotDisplayData.tweenKeyList.push(false);
                }
                var tDeformVectices = arrayList[this.readVaruint()];
      ...
      for (i = 0; i < tDrawOrderAniLen; i++) {
        ...
        for (j = 0; j < tDrawOrderLen; j++) {
          ...
          tDrawOrderData.drawOrder = arrayList[this.readVaruint()];
      ...
      for (i = 0; i < tEventAniLen; i++) {
        ...
        for (j = 0; j < tEventLen; j++) {
          ...
          tEventData.name = strList[this.readVaruint()];
          tEventData.intValue = tByte.readInt32();
          tEventData.floatValue = tByte.readFloat32();
          tEventData.stringValue = strList[this.readVaruint()];
          tEventData.time = 1000 * tByte.readFloat32();
      ...
      if (tAttachmentLen > 0) {
        this.attachmentNames = new Array(tAttachmentLen);
        for (i = 0; i < tAttachmentLen; i++) {
          this.attachmentNames[i] = strList[this.readVaruint()];
        }
      };
      ...
      for (i = 0; i < tBoneSlotLen; i++) {
        ...
        tDBBoneSlot.name = strList[this.readVaruint()];
        tDBBoneSlot.parent = strList[this.readVaruint()];
        let blendValue = tByte.readUint8();
        var indexValue
        if ((blendValue & 0x80) == 0) {
          indexValue = tByte.readUint8();
          tDBBoneSlot.attachmentName = strList[this.readVaruint()];
        } else {
          indexValue = -1;
          blendValue = blendValue & 0x7f;
        }
        tDBBoneSlot.srcDisplayIndex = tDBBoneSlot.displayIndex = indexValue;
        tDBBoneSlot.blendMode = blendValue == 0 ? 'normal' : 'add';
      ...
      for (i = 0; i < tSkinDataLen; i++) {
        ...
        tSkinData.name = strList[this.readVaruint()];
        var tSlotDataLen = tByte.readUint8();
        for (j = 0; j < tSlotDataLen; j++) {
          var tSlotData = new SlotData();
          tSlotData.name = strList[this.readVaruint()];
          tDBBoneSlot = this.boneSlotDic[tSlotData.name];
          var tDisplayDataLen = tByte.readUint8();
          for (k = 0; k < tDisplayDataLen; k++) {
            var tDisplayData = new SkinSlotDisplayData();
            this.skinSlotDisplayDataArr.push(tDisplayData);
            tDisplayData.name = strList[this.readVaruint()];
            tDisplayData.attachmentName = strList[this.readVaruint()];
            tDisplayData.transform = this.readTransform(tByte);
            tSlotData.displayArr.push(tDisplayData);
            var bit = tByte.readUint8();
            tDisplayData.type = bit & 1;
            if (tDisplayData.type == 0) {
              tDisplayData.width = tByte.readUint16();
              tDisplayData.height = tByte.readUint16();
            }
            if ((bit & 2) != 0) {
              tDisplayData.bones = arrayList[this.readVaruint()];
            }
            if ((bit & 4) != 0) {
              tDisplayData.uvs = arrayList[this.readVaruint()];
            }
            if ((bit & 8) != 0) {
              tDisplayData.weights = arrayList[this.readVaruint()];
            }
            if ((bit & 16) != 0) {
              tDisplayData.triangles = arrayList[this.readVaruint()];
            }
            if ((bit & 32) != 0) {
              tDisplayData.vertices = arrayList[this.readVaruint()];
            }
            if ((bit & 64) != 0) {
              tDisplayData.lengths = arrayList[this.readVaruint()];
            }
          }
          tSkinData.slotArr[j] = tSlotData;
        }
    ...
    _textureComplete03() {
      this._parsePublicExtData03();
      var strList = this.strList;
      var tSrcTexturePath = this._loadList[0];
      for (var i = 0, n = this._loadList.length; i < n; i++) {
        var tTextureName = this._loadList[i];
        var tTexture = this._textureDic[tTextureName] = Laya.ILaya.Loader.getRes(tTextureName);
      }
      var tByte = this.reader;
      var dataView = tByte._d_;
      var tTextureLen = tByte.readInt32();
      var pos = tByte._pos_;
      for (i = 0; i < tTextureLen; i++) {
        var tTexture = this._mainTexture;
        var tTextureName = strList[this.readVaruint(pos)];
        pos = tByte._pos_;
        ...
        var tX = dataView.getUint16(pos, true);
        var tY = dataView.getUint16(pos + 2, true);
        var tWidth = dataView.getUint16(pos + 4, true);
        var tHeight = dataView.getUint16(pos + 6, true);
        var tTempleData = dataView.getUint16(pos + 8, true);
        var tFrameX = isNaN(tTempleData) ? 0 : tTempleData;
        tTempleData = dataView.getUint16(pos + 10, true);
        var tFrameY = isNaN(tTempleData) ? 0 : tTempleData;
        tTempleData = dataView.getUint16(pos + 12, true);
        var tFrameWidth = isNaN(tTempleData) ? tWidth : tTempleData;
        tTempleData = dataView.getUint16(pos + 14, true);
        pos += 16;
        var tFrameHeight = isNaN(tTempleData) ? tHeight : tTempleData;
    ...
    readVaruint(pos = -1) {
      var byte = this.reader;
      if (pos == -1) pos = byte._pos_;
      var u8Bin = byte._u8d_;
      let b = u8Bin[pos++];
      let value = b & 0x7F;
      if (b & 0x80) {
        b = u8Bin[pos++];
    ...
    readTransform(tByte) {
      ...
      var keys = Templet.transformKey;
      var transformType = tByte.readUint8();
      var bitMask = Templet.bitMask;
      for (var l = 0; l < 7; l++) {
        if ((bitMask[l] & transformType) != 0) {
          transform[keys[l]] = tByte.readFloat32();
        }
      }
      transform.skY = transform.skX;
      return transform;
    }
    readStr(byte) {
      var u = byte._u8d_;
      var pos = byte._pos_;
      var len = u[pos] + u[pos + 1] * 256;
      pos += 2;
      var arr = new Uint8Array(u.buffer, pos, len);
      byte._pos_ = pos + len;
    ...
    _textureComplete() {
      ...
      Laya.timer.frameOnce(1, this, this._parsePublicExtData)
      // this._parsePublicExtData();
    }
    _parsePublicExtData() {
      var i = 0, j = 0, k = 0, l = 0, n = 0;
      for (i = 0; i<this._anis.length; i++) {
        if (!this._anis[i]) return;
      }
  ...
  Templet.slotInterpolationMethod = null;
  Templet.bitMask = [1, 2, 4, 8, 16, 32, 64, 128];
  Templet.transformKey = ['scX', 'scY', 'skX', 'x', 'y', 'skewX', 'skewY'];
